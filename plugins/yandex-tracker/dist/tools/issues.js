...
function formatIssue(issue) {
    ...
    if (issue.queue)
        md += `**Queue:** ${issue.queue.display ?? issue.queue.key ?? "N/A"}\n`;
    if (issue.parent?.key)
        md += `**Parent:** ${issue.parent.key}\n`;
    if (issue.tags?.length)
        md += `**Tags:** ${issue.tags.join(", ")}\n`;
    md += `\n`;
...
export function registerIssueTools(server, client) {
    server.registerTool("yandex_tracker_get_issue", {
        title: "Get Issue",
...
Examples:
  ...
  - "Get PROJ-123 as JSON" -> issue_key="PROJ-123", response_format="json"`,
        ...
        const issue = await client.request(`/issues/${args.issue_key}`);
        const text = args.response_format === "json"
            ? JSON.stringify(issue, null, 2)
            : formatIssue(issue);
        return { content: [{ type: "text", text }] };
    });
    server.registerTool("yandex_tracker_create_issue", {
        title: "Create Issue",
...
Examples:
  ...
  - "Create subtask for PROJ-100" -> queue="PROJ", summary="...", parent="PROJ-100"`,
        ...
        const issue = await client.request("/issues", { method: "POST", body: JSON.stringify(body) });
        return { content: [{ type: "text", text: `Issue ${issue.key} created\n\n${formatIssue(issue)}` }] };
    });
    server.registerTool("yandex_tracker_update_issue", {
        title: "Update Issue",
...
Examples:
  ...
  - "Assign to john" -> issue_key="PROJ-123", assignee="john"`,
        ...
        const issue = await client.request(`/issues/${issue_key}`, { method: "PATCH", body: JSON.stringify(body) });
        return { content: [{ type: "text", text: `Issue ${issue_key} updated\n\n${formatIssue(issue)}` }] };
    });
    server.registerTool("yandex_tracker_search_issues", {
        title: "Search Issues",
...
Examples:
  ...
  - "My tasks" -> query="Assignee: me() AND Status: !closed"`,
        ...
        const issues = await client.request(`/issues/_search?${qp.toString()}`, { method: "POST", body: JSON.stringify(body) });
